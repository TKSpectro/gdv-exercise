...
namespace
{
    class CApplication : public IApplication
    {
        ...
        private:
            ...
            EState m_CurrentState = EState::Roll;
...
namespace
{
    ...
    bool CApplication::InternOnUpdate()
    {
        ...
        // -----------------------------------------------------------------------------
        Eye[0] =  m_xPosition + 2.5;  At[0] = m_xPosition + 2.5;    Up[0] = 0.0f;
        Eye[1] =  4.0f;         At[1] = 3.0f;           Up[1] = 1.0f;
    ...
    bool CApplication::InternOnFrame()
    {
        ...
        if(m_xPosition >= m_xMax)
        {
            std::cout << "You have won" << std::endl;
            std::cout << "Points:" << m_xMax / m_obstacleDivider << std::endl;
        }
        ...
        for(int i = 0; i < m_xMax; ++i)
        {
            ...
            // -----------------------------------------------------------------------------
            GetTranslationMatrix(i, -1.5f, 0.0f, WorldMatrix);
        ...
        // Draw obstacles
        for(int i = 0; i < m_xMax / m_obstacleDivider - 1; ++i)
        {
            // -----------------------------------------------------------------------------
            // Set the position of the mesh in the world and draw it.
            // -----------------------------------------------------------------------------
            GetTranslationMatrix(i * m_obstacleDivider + m_obstacleDivider, -0.5f, 0.0f, WorldMatrix);
        ...
        // Rotate Ball
        GetRotationZMatrix(m_ballRot, rotMatrix);
        m_ballRot -= 2;
        m_ballRot = fmod(m_ballRot, 360);

        // Move ball
        GetTranslationMatrix(m_xPosition, m_yPosition, 0.0f, transMatrix);
        m_xPosition += m_xDelta;
        ...
        switch(m_CurrentState)
        {
            ...
            case EState::Up:
            {
                ...
                m_yPosition += m_yDelta;

                if(m_yPosition >= m_yMax)
                {
                    m_CurrentState = EState::Down;
                    m_pressedJump = false;
                }
            }
            break;
        ...
        // Set WorldMatrix
        MulMatrix(rotMatrix, transMatrix, WorldMatrix);
        SetWorldMatrix(WorldMatrix);
